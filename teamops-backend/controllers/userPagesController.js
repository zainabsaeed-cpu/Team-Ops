...
exports.getSettings = async (req, res) => {
    try {
        ...
        res.json({
            ...
            inviteLink: workspace?.inviteCode ? `${process.env.FRONTEND_URL || 'http://localhost:5173'}/join?code=${encodeURIComponent(workspace.inviteCode)}` : '',
...
exports.updateSettings = async (req, res) => {
    try {
        ...
        res.json({
            ...
            inviteLink: freshWorkspace?.inviteCode ? `${process.env.FRONTEND_URL || 'http://localhost:5173'}/join?code=${encodeURIComponent(freshWorkspace.inviteCode)}` : '',
            currentUserRole: member?.role || 'member',
            notifyEmail: user?.notifyEmail !== false,
            notifyPush: user?.notifyPush !== false,
        });
